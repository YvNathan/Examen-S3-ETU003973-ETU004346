...
BEGIN
    -- Gestionnaire d'erreur : Annule tout en cas de problème SQL
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Erreur interne : La livraison n\'a pas pu être créée.';
    END;

    -- Début du bloc atomique
    START TRANSACTION;
...
BEGIN
    ...
    DECLARE poids_Kg DECIMAL(10,2);
    DECLARE prixTotal DECIMAL(10,2);
    DECLARE nbStatut INT;

    -- Vérifie que la livraison est en attente
    SELECT COUNT(*)
    INTO nbStatut
    FROM lvr_livraisonStatut
    WHERE idLivraison = p_idLivraison
    ...
    IF nbStatut = 0 THEN
        ...
        SET MESSAGE_TEXT = 'Livraison inexistante ou non en cours';
    END IF;
    ...
    IF prixKg IS NULL OR poids_Kg IS NULL THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Données manquantes pour la livraison';
    END IF;
