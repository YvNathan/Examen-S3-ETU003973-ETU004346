...
CREATE
OR REPLACE VIEW v_lvr_colisDisponibles AS
SELECT
    *
FROM
    lvr_colis
...
CREATE
OR REPLACE VIEW v_lvr_benefices AS
SELECT
    l.id AS idLivraison,
    l.dateLivraison,
    c.descrip AS colis,
...
FROM
    ...
    AND ls.dateStatut = (
        SELECT
            MAX(dateStatut)
        FROM
            lvr_livraisonStatut ls2
        WHERE
            ls2.idLivraison = l.id
    )
...
-- bénéfices par jour
CREATE
OR REPLACE VIEW v_lvr_benefices_jour AS
SELECT
...
-- Bénéfices par mois
CREATE
OR REPLACE VIEW v_lvr_benefices_mois AS
SELECT
...
-- Bénéfices par année
CREATE
OR REPLACE VIEW v_lvr_benefices_annee AS
SELECT
    YEAR (dateLivraison) AS annee,
    COUNT(*) AS nb_livraisons,
    SUM(IFNULL (chiffreAffaires, 0)) AS ca_total,
...
CREATE
OR REPLACE VIEW v_lvr_benefices_periode AS
SELECT
...
CREATE
OR REPLACE VIEW v_getStatusLivraison AS
SELECT
    l.id AS idLivraison,
    c.descrip AS colis,
    ls.dateStatut,
...
CREATE
OR REPLACE VIEW v_lvr_benefices_vehicules_details AS
SELECT
    v.id AS idVehicule,
    v.modele,
    v.immatriculation,
...
CREATE
OR REPLACE VIEW v_lvr_benefices_vehicules AS
SELECT
    idVehicule,
    modele,
    immatriculation,
...
FROM
    v_lvr_benefices_vehicules_details
GROUP BY
    idVehicule,
    modele,
    immatriculation
